...
- **AI-attribution phrase list synced into the shell hook.** New
  `forge-gen-attribution-patterns` CLI + `attribution_parity`
  pre-commit step keep `block_claude_attribution.sh`'s managed block in
  lockstep with `AI_ATTRIBUTION_PATTERNS` — the same mechanism as
  `forge-gen-commit-types`. The Python-only bare-vendor-token backstop
  stays Python-only (documented residual).
...
- **`config.read_tool_forge_section`** — the one `[tool.forge.<section>]`
  ...
  `verify_test_naming`, and `config` itself now route through it.
