...
- **`block_claude_attribution.sh` now blocks bare `generated with` /
  `generated by` phrasing regardless of vendor co-occurrence** (parity
  with the Python validator's phrase list — the previous hook required
  `claude` within a few characters). Benign prose like "generated with
  care" in a commit message now trips the hook: reword, or a human uses
  the `!` escape hatch.
