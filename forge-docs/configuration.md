...
| `cadence_mode` | `commit` | Who carries the truly-all cadence: `commit` (stale stamp escalates the commit), `advisory` (warn only), `external` (a scheduled CI job owns it; warn at 2x the window). | CI is your testing fleet — see the smart-test doc's "Who carries the cadence". |
