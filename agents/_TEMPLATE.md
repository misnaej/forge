...
| Role | Examples | Tools |
|---|---|---|
| **Reporter** | design-checker, security-checker, knowledge-search | `Read`, `Grep`, `Glob`, **read-only `Bash`** (+ `WebFetch` / `WebSearch` if grounding, + `Task` if delegating). Read-only `Bash` is whitelisted for: `git rev-parse` (SHA capture), `git diff` (PR diff read), `git branch` (current branch), `gh pr view --json` (PR-context capture), `forge-precommit --freshness` (log freshness verdicts; runs no steps), `forge-audit-*` runs (log refresh). No writes, no commits, no `gh pr comment`. |
