...
- `_publish_assembly_pr(root: Path, date: str, version: str, bare_version: str, *, draft: bool) -> int` _(internal)_ — Branch, stage, commit, push the assembly and open its PR.
- `_push_and_open_pr(root: Path, branch: str, version: str, *, draft: bool) -> int` _(internal)_ — Push the assembly branch and open its PR, deferring to race winners.
...
- `_open_resync_pr_url(root: Path) -> str | None` _(internal)_ — Return the URL of an already-open resync PR, or ``None``.
