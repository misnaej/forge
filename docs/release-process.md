...
| `forge-release` refuses a repo whose tags another tool owns: a manifest that declares a version (`forge-next-prep --tag`), or tag-per-merge (`[tool.forge.release].auto = "merge"` — `forge-changelog auto-tag`), which covers forge's own repo although its manifest declares none | `release._wrong_release_model_error` | `tests/test_release.py::test_wrong_release_model_error_refuses_tag_per_merge_repo` / `::test_wrong_release_model_error_none_for_version_less_manifest_without_auto_merge` |
