...
| A consumer (no manifest of its own) is judged on **content**: the cache slot's hook set against the marketplace clone the pin resolves to — the declared version identifies nothing under fragments mode, and the advisory names the missing hooks plus a slot-deletion remedy, never `/plugin update` | `version_surfaces._consumer_cache_status` via `doctor._check_plugin_cache_skew` | `tests/test_version_surfaces.py::test_plugin_cache_status_consumer_reports_hooks_the_cache_lacks` / `tests/test_doctor.py::test_plugin_cache_skew_names_missing_hooks_for_a_consumer` |
