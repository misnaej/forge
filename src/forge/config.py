...
same key (so the hook has no ``forge-scripts`` dependency at
...
# Single-branch flow: every CLI / hook points at one protected base
# branch. Backwards-compatible with every existing consumer repo that
# has no ``[tool.forge]`` block.
...
class ForgeConfig:
    ...
    Attributes:
        base_branch: Name of the protected base branch (typically
            ``"main"``).
...
def load_config(repo_root: Path) -> ForgeConfig:
    ...
    Returns:
        Populated :class:`ForgeConfig`. With no overrides,
