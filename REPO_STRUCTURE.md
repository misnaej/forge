...
1. **CLI Modules**
   ...
   - pr_squash_comment.py: `forge-pr-squash-comment` — validates + posts the squash-merge body (bullets only; the PR title is the squash title) and keeps that comment the PR's newest; canonical `CONVENTIONAL_COMMIT_TYPES` source
