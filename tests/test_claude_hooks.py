...
def test_gh_guards_block_a_repo_override_before_the_subcommand(
    ...
    command). What is pinned here is the ANCHOR, not what follows it:
    the create guard now refuses every raw invocation outright, so a
    block proves the override did not hide the subcommand from the
    match — which is the only thing this test is about.
    """
...
def test_unverified_pr_create_blocks_raw_create() -> None:
    """A raw `gh pr create` is refused, naming `forge-pr-create` as the replacement.

    Verification moved entirely into that CLI (see its module docstring
    and this hook's header comment) — the guard's only remaining job is
    refusing the raw form and pointing at the sanctioned one.
    """
    proc = _run_hook_proc(_UNVERIFIED_PR_CREATE, "gh pr create --title x")
    assert proc.returncode == 2
    assert "forge-pr-create" in proc.stderr


def test_unverified_pr_create_allows_forge_pr_create_cli() -> None:
    """The replacement CLI's own invocation passes through untouched.

    The guard's job shrank to a name check on `gh pr create`; if that
    pattern were sloppy enough to also catch `forge-pr-create`, it would
    block the very command it tells the caller to run instead.
    """
    assert (
        _run_hook(
            _UNVERIFIED_PR_CREATE,
            "forge-pr-create --base main --title x --body-file body.md",
        )
        == 0
    )


def test_unverified_pr_create_allows_skip_gate_env_prefix() -> None:
    """The explicit `FORGE_SKIP_WRAPUP_GATE=1` prefix still allows a raw create.

    Sanctioned bypass, unchanged by the move: a human running it directly,
    or the user explicitly requesting the skip — never the agent's own
    judgment.
    """
    assert (
        _run_hook(
